...
FORCE_INLINE inline QString hexstr8(uint8_t hex_number)
{
    return QString::number(hex_number, 16).rightJustified(2, '0');
}

FORCE_INLINE inline QString hexstr16(uint16_t hex_number)
{
    return QString::number(hex_number, 16).rightJustified(4, '0');
}

FORCE_INLINE inline QString hexstr(uint8_t* array, size_t size, QString separator = " ")
{
    QString ret;
...
FORCE_INLINE inline const QString& register_name(uint16_t address)
{
...
FORCE_INLINE inline const QString& memory_abbreviation(uint16_t address)
{
