...
static const std::array<const InstrInfo, 256> INSTR_TABLE =
{{
    {1, 4,  4,  &Cpu::op_00},
    {3, 12, 12, &Cpu::op_01},
    {1, 8,  8,  &Cpu::op_02},
...
const std::array<const InstrInfo, 256> CB_INSTR_TABLE =
{{
    {2, 8,  8,  &Cpu::op_CB_00},
    {2, 8,  8,  &Cpu::op_CB_01},
    {2, 8,  8,  &Cpu::op_CB_02},
...
void Cpu::execute(const uint8_t* instruction)
{
    ...
    if (!is_halted && !is_stopped)
    {
        ...
        if (branch_taken)
            total_tcycles += op_info->cycles_on_action;
        else
            total_tcycles += op_info->cycles_on_no_action;
    }
    else
    {
